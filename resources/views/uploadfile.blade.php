...
<body class="bg-gradient-to-r from-purple-600 to-indigo-500 min-h-screen flex items-center justify-center">
    ...
    <div class="bg-white rounded-3xl shadow-2xl p-10 max-w-md w-full animate__animated animate__fadeInDown">
        ...
        <h2 class="text-3xl font-bold text-indigo-900 mb-6 text-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 inline-block mr-2 text-yellow-400" fill="none"
                viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round"
                    d="M3 7v4h4M21 7v4h-4M3 17v-4h4M21 17v-4h-4M12 3v4M12 17v4M7 12h4M13 12h4" />
            </svg>
            Upload RDF File
        </h2>
    ...
    <script>
        ...
        document.getElementById('uploadForm').addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch(`${window.apiBaseUrl}/files/upload`, {
                    method: 'POST',
                    headers: {
